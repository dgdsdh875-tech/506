    ...
    db.products.forEach(p => {
        ...
        if (activeCategoryFilter === 'الكل' || p.category === activeCategoryFilter) {
            ...
            if(cartItem) {
                ...
                posActionHtml = `<button class="btn-3d" style="margin-top: 10px;" onclick="triggerFlip(this, () => addToCart(${p.id}))"><i class="fas fa-cart-plus"></i> أضف للسلة</button>`;
            }

            posGrid.innerHTML += `
                <div class="card" style="display: flex; flex-direction: column; justify-content: space-between;">
                    <div onclick="addToCart(${p.id})" style="cursor: pointer;">
                        <img src="${p.img}" alt="صورة">
                        <h3>${p.name}</h3>
                        ${catBadge}
                        <div class="price">${p.price.toLocaleString()} د.ع</div>
